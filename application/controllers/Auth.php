...
class Auth extends MY_Controller {
	...
	public function login(){
		...
		public function verify(){
			...
			$result = $this->users_model->email_verification($verification_id);
			if($result){
		...
		public function forgot_password(){
			...
			if($this->input->post('submit')){
				...
				$email = $this->input->post('email');
				$response = $this->users_model->check_user_mail($email);

				if($response){

					$rand_no = rand(0,1000);
					$pwd_reset_code = md5($rand_no.$response['user_id']);
					$this->users_model->update_reset_code($pwd_reset_code, $response['user_id']);
		...
		public function reset_password($id=0){
			...
			if($this->input->post('submit')){
				...
				if ($this->form_validation->run() == FALSE) {
					...
					$this->users_model->reset_password($id, $new_password);
					$this->session->set_flashdata('success','New password has been Updated successfully.Please login below');
					redirect(base_url('admin/auth/login'));
				}
			}
			else{
				$result = $this->users_model->check_password_reset_code($id);
