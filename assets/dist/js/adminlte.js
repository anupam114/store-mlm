...
(async () => {
	let fileInputs = document.querySelectorAll(".form-file");
	let formTexts = document.querySelectorAll(".file-text");

	fileInputs.forEach((input, index) => {
		input.addEventListener("change", (e) => {
			let files = e.target.files;
			if (files.length === 1) {
				formTexts[index].innerHTML = files[0].name;
        return;
			}
      formTexts[index].innerHTML =`${files.length} Files chosen`;
		});
	});
})();
